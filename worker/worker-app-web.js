    ...
    "roots": [
        {
            ...
            "auth": { "firahsrisab": "4321" } /* Remove double slash before "auth" to activate id password protection */
        },
        // add more drives, shared drives, folders like above json object with , separated.
    ]
};
...
<head>
  ...
  <!-- Website UI : app-web.js -->
  <script src="https://cdn.jsdelivr.net/gh/basirsharif/worker-app/worker/app-web.js"></script>
</head>
<body>
</body>
...
const download_error = `<html>
  <head>
     <title>Error on File or Folder</title>
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <link href="https://fonts.googleapis.com/css?family=Quicksand:300,400&display=swap" rel="stylesheet">
     <style>body {margin: 0;padding: 0;width: 100%;height: 100%;background-color: #000000;display: table;font-weight: 100;font-family: 'Quicksand', sans-serif;} .container {text-align: center;display: table-cell;vertical-align: middle;} .content {text-align: center;display: inline-block;}.message {font-size: 64px;margin-bottom: 40px;margin-left: 100px;margin-right: 100px;}.notice {font-size: 30px;margin-bottom: 0px;margin-left: 10px;margin-right: 10px;font-weight: 100;}.warning {font-size: 20px;margin-top: 10px;margin-left: 10px;margin-right: 10px;font-weight: 100;}.copyright {font-size: 20px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;font-weight: 100;} a {text-decoration: none;color: #ffffff;}</style>
  </head>
  <body>
     <div class="container">
        <div class="content">
           <div class="message"><a >File Error</div>
           <div class="notice"><a id="reason">Sorry, you can't view or download this file at this time.</a></div>
           <div class="warning"><a id="name">This is possible because of several reasons, try downloading other files from the server or try again later. <br>There is no solution available for this kind of error.</div>
        </div>
     </div>
  </body>
</html>`;

const disable_download=`<script>var host = window.location.href + "?a=view"; location.replace(host)</script>`;
...
const DriveFixedTerms = new(class {
    default_file_fields = 'parents,id,name,mimeType,modifiedTime,createdTime,fileExtension,size,videoMediaMetadata';
    gd_root_type = {
        user_drive: 0,
        share_drive: 1,
        sub_folder: 2
...
async function handleRequest(request) {
    ...
    } else if (path.toLowerCase() == '/developer') {
        return Response.redirect("https://basirsharif.github.io", 301);
    }
...
class googleDrive {
    ...
    async down(id, range = '', inline = false) {
        ...
        if (second_domain_for_dl == 'true') {
            return new Response(await disable_download, {
                headers: {
                    "content-type": "text/html;charset=UTF-8",
                },
            })
            ...
            return new Response(await download_error, {
                headers: {
                    "content-type": "text/html;charset=UTF-8",
                },
            })
    ...
    async _file(path) {
        ...
        params.fields = "files(id, name, mimeType, size ,createdTime, modifiedTime, iconLink, thumbnailLink,videoMediaMetadata)";
    ...
    async _ls(parent, page_token = null, page_index = 0) {
        ...
        params.fields = "nextPageToken, files(id, name, mimeType, size , modifiedTime,videoMediaMetadata)";
    ...
    async _findDirId(parent, name) {
        ...
        params.fields = "nextPageToken, files(id, name, mimeType, videoMediaMetadata)";
        url += '?' + this.enQuery(params);
        let requestOption = await this.requestOption();
        let response = await fetch(url, requestOption);
        let obj = await response.json();
